...
Still uncovered: the execution engine, approval resolution and quorum,
idempotency replay, and the payment rails. Those guarantees are verifiable by
reading the source but not yet by running a test. Closing that gap is roadmap
**P2-T5**, and it is scheduled deliberately: the storage layer beneath them
changes in Phase 2, so the tests are written against the shape they will keep.
